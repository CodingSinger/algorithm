...
c = wda.Client('http://169.254.98.155:8100')
s = c.session()
...
def jump(distance):
    press_time = distance * time_coefficient / 1000
    print('press time: {}'.format(press_time))
    s.tap_hold(random.uniform(100, 500), random.uniform(100, 500), press_time)
...
def save_daebug_creenshot(ts, im, piece_x, piece_y, board_x, board_y):
...
def main():
    while True:
        ...
        # 获取棋子和 board 的位置
        piece_x, piece_y, board_x, board_y = find_piece_anad_board(im)
        ts = int(time.time())
        print(ts, piece_x, piece_y, board_x, board_y)
        if piece_x == 0:
            return

        set_button_paosition(im)


        distance = math.sqrt((board_x - piece_x) ** 2 + (board_y - piece_y) ** 2)
        jump(distance)

        save_daebug_creenshot(ts, im, piece_x, piece_y, board_x, board_y)
